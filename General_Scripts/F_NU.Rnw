...
plotName = "F_NU"
outDir = "Clustering_F_NU"
dir.create(paste(getwd(),"/",outDir,sep=""))
@

%\author{Lindsay Rutter}
\title{Cluster Analysis of Wasp F NU}
...
#L120 (157691, 12)
L120 = y[ , c("F.1", "F.2", "F.3", "F.4", "F.5", "F.6", "NU.1", "NU.2", "NU.3", "NU.4", "NU.5", "NU.6")]
...
L120 = mutate(L120, mean = (F.1+F.2+F.3+F.4+F.5+F.6+NU.1+NU.2+NU.3+NU.4+NU.5+NU.6)/ncol(L120), stdev = RowSD(cbind(F.1,F.2,F.3,F.4,F.5,F.6,NU.1,NU.2,NU.3,NU.4,NU.5,NU.6)))
...
\begin{center}
\foreach \i in {2,3,...,15} {
 \includegraphics[width=0.99\textwidth]{Clustering_F_NU/F_NU_\i}
  \captionof{figure}{Parallel coordinate plot of the normalized F_NU read counts across \i\ clusters}
}
\end{center}
