...
plotName = "DR_NR"
outDir = "Clustering_DR_NR"
dir.create(paste(getwd(),"/",outDir,sep=""))
@

%\author{Lindsay Rutter}
\title{Cluster Analysis of Wasp DR NR}
...
#L120 (157691, 12)
L120 = y[ , c("DR.1", "DR.2", "DR.3", "DR.4", "DR.5", "DR.6", "NR.1", "NR.2", "NR.3", "NR.4", "NR.5", "NR.6")]
...
L120 = mutate(L120, mean = (DR.1+DR.2+DR.3+DR.4+DR.5+DR.6+NR.1+NR.2+NR.3+NR.4+NR.5+NR.6)/ncol(L120), stdev = RowSD(cbind(DR.1,DR.2,DR.3,DR.4,DR.5,DR.6,NR.1,NR.2,NR.3,NR.4,NR.5,NR.6)))
...
\begin{center}
\foreach \i in {2,3,...,15} {
  \includegraphics[width=0.99\textwidth]{Clustering_DR_NR/DR_NR_\i}
  \captionof{figure}{Parallel coordinate plot of the normalized DR_NR read counts across \i\ clusters}
}
\end{center}
